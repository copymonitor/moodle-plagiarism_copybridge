...
class CopyBridgeHttpRequest
{
    ...
    // Modify it so that it can be called with curl (#10521)
    function sendWithCurl(){

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->m_host.':'.$this->m_port.$this->m_uri);

        curl_setopt($ch, CURLINFO_HEADER_OUT, true);
        curl_setopt($ch, CURLOPT_ENCODING, "gzip,deflate");
        curl_setopt($ch, CURLOPT_TIMEOUT, $this->m_timeout);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $this->m_post_vars);
        curl_setopt($ch, CURLOPT_HEADER, 0);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_USERAGENT, PHP_OS." php ".PHP_VERSION);
